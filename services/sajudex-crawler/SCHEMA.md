...
- `curated_people`: Sajudex 앱의 명식록(유명인 도감)에 사용될 정제 인물 데이터 (참고: 지인 관리용 '인연록'은 메인 앱에 별도 테이블로 구성)
...
- 사주 검색에 필요한 일주/월주/년주 정보는 한 인물당 6개의 판(본원, 차력, 부허자 등)을 생성하여 `person_saju_plates` (1:N 테이블)에 저장합니다. "어느 판에 있든 xx일주인 명식 찾기"와 같은 통합 검색의 성능 최적화를 위한 구조입니다.
